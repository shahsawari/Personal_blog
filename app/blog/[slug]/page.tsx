...
function renderNode(node: RichNode, index: number) {
  ...
  if (node.type === 'image' && node.attrs) return <Image key={index} src={String((node.attrs as Record<string, unknown>).src ?? '')} alt={String((node.attrs as Record<string, unknown>).alt ?? '')} width={1200} height={800} sizes="(max-width: 768px) 100vw, 768px" className="my-8 h-auto w-full rounded-2xl" />
...
export default async function PostPage({ params }: Props) {
  ...
  return <main className="min-h-screen bg-background px-5 py-8 text-foreground md:px-8 md:py-12" dir="rtl"><article className="mx-auto max-w-3xl"><Link href="/blog" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">بازگشت به نوشته‌ها <ArrowLeft className="size-4" /></Link><header className="py-16"><div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground"><span>{post.publishedAt?.toLocaleDateString('fa-IR')}</span><span className="size-1 rounded-full bg-border" /><span className="flex items-center gap-1"><Clock3 className="size-4" /> {post.readingTime} دقیقه مطالعه</span></div><h1 className="mt-6 text-balance text-4xl font-bold leading-[1.45] tracking-tight md:text-6xl">{post.title}</h1>{post.excerpt && <p className="mt-6 text-pretty text-lg leading-9 text-muted-foreground">{post.excerpt}</p>}</header>{post.coverImage && <Image src={post.coverImage} alt={post.title} width={1600} height={900} priority sizes="(max-width: 768px) 100vw, 768px" className="mb-12 aspect-[16/9] w-full rounded-3xl object-cover" />}<div className="prose prose-lg max-w-none dark:prose-invert">{content.content?.map(renderNode)}</div><footer className="mt-16 border-t border-border pt-8"><Link href="/blog" className="text-sm font-bold text-primary">مشاهده همه نوشته‌ها</Link></footer></article></main>
}
