...
export default function PostEditor({ initialContent, onChange }: { initialContent?: Record<string, unknown>; onChange: (content: Record<string, unknown>) => void }) {
  ...
  async function insertImage(file: File) { setUploading(true); try { const body = new FormData(); body.append('file', file); const response = await fetch('/api/upload', { method: 'POST', body }); const data = await response.json(); if (!response.ok) throw new Error(data.error); editor?.chain().focus().setImage({ src: data.url, alt: file.name }).run() } finally { setUploading(false) } }
